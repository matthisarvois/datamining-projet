...
class CustomerFeatureEngineer(BaseEstimator, TransformerMixin):
    ...
    def _transform_single_customer(self, customer_data: dict) -> dict:
        ...
        if self._is_fitted:
            features["favorite_category_encoded"] = self.category_encoder.transform([favorite_cat])[
                0
            ]
        else:
    ...
    def _transform_dataframe(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        X_transformed["avg_order_value"] = X_transformed["total_spent"] / X_transformed[
            "total_orders"
        ].clip(lower=1)
        ...
        if "favorite_category" in X_transformed.columns and self._is_fitted:
            try:
                ...
                print("      Catégories inconnues détectées, utilisation de 0 par défaut")
                known_categories = set(self.category_encoder.classes_)
                X_transformed["favorite_category_encoded"] = categories.apply(
                    lambda x: self.category_encoder.transform([x])[0]
    ...
    def _add_rfm_segments(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            df["recency_quartile"] = (
                pd.qcut(df["days_since_last_order"], q=4, labels=False, duplicates="drop") + 1
            )
        except ValueError:
            df["recency_quartile"] = (
                pd.cut(df["days_since_last_order"], bins=4, labels=False, duplicates="drop") + 1
            )

        try:
            freq_quartiles = pd.qcut(df["total_orders"], q=4, labels=False, duplicates="drop")
            df["frequency_quartile"] = 4 - freq_quartiles
        except ValueError:
            freq_bins = pd.cut(df["total_orders"], bins=4, labels=False, duplicates="drop")
            df["frequency_quartile"] = 4 - freq_bins

        try:
            mon_quartiles = pd.qcut(df["total_spent"], q=4, labels=False, duplicates="drop")
            df["monetary_quartile"] = 4 - mon_quartiles
        except ValueError:
            mon_bins = pd.cut(df["total_spent"], bins=4, labels=False, duplicates="drop")
...
class RecommendationFeatureEngine:
    ...
    def create_customer_features(
        ...
        customer_orders = (
            ...
            .rename(
                columns={"order_id": "total_orders", "order_purchase_timestamp": "last_order_date"}
            )
        )
        ...
        # ============================================
        customer_spending = (
            order_items_df.merge(orders_df[["order_id", "customer_id"]], on="order_id")
        ...
        if not reviews_df.empty:
            customer_reviews = (
                reviews_df.merge(orders_df[["order_id", "customer_id"]], on="order_id")
                .groupby("customer_id")["review_score"]
                .mean()
                .to_frame("avg_review_score")
            )
        else:
        ...
        customer_features["favorite_category"] = customer_features["favorite_category"].fillna(
            "unknown"
        )
...
def load_and_prepare_data(
    ...
    print(
        f"   Chargé: {len(customers)} clients, {len(orders)} commandes, "
